...
# Embedded Profiles Last Modified (UTC): 2026-09-14T01:40:02Z
...
(allow file-read*
    ...
    (home-subpath "/.local/bin")                                      ;; User-installed binaries in the XDG binary root; writes stay denied because it is on PATH.
...
;; Directory-entry creation for the XDG base directory roots.
;; See https://specifications.freedesktop.org/basedir/latest/
(allow file-write-create                                              ;; #safehouse-test-id:xdg-root-create#
    (home-literal "/.cache")                                          ;; $XDG_CACHE_HOME default; parent of ~/.cache/<tool>.
    (home-literal "/.config")                                         ;; $XDG_CONFIG_HOME default; parent of ~/.config/<tool>.
    (home-literal "/.local")                                          ;; Parent of the ~/.local/* roots below.
    (home-literal "/.local/share")                                    ;; $XDG_DATA_HOME default; parent of ~/.local/share/<tool>.
    (home-literal "/.local/state")                                    ;; $XDG_STATE_HOME default; parent of ~/.local/state/<tool>.
)
...
;; Docker Desktop: Anonymous pulls.
;; - When `~/.docker/config.json` sets `"credsStore": "desktop"`, Docker uses
;;   the `docker-credential-desktop` helper which must be able to create its own
;;   state directory to start and answer "credentials not found".
;; - Authenticated pulls (and `docker login` inside the sandbox) additionally
;;   require `--enable=keychain` permissions.
(allow file-read-metadata
    (home-literal "/Library/Containers")                                   ;; Traversal only; the helper stats this before creating its own subdirectory.
)
(allow file-read* file-write*
    (home-subpath "/Library/Containers/com.docker.docker")                 ;; docker-credential-desktop state/log dir, created on every invocation.
)
(allow mach-lookup
    (global-name "com.apple.SecurityServer")                               ;; Security framework lookup; without it the helper errors (-50) instead of reporting "not found".
)
...
safehouse_project_version_embedded="0.12.0"
