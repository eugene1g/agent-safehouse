...
# Embedded Profiles Last Modified (UTC): 2026-09-03T04:20:06Z
...
;; The Deno install root holds the runtime and `deno install` binaries, so it is
;; read-only. DENO_DIR (the module/build cache used by ordinary project work)
;; lives under the cache paths below and stays writable.
(allow file-read*
    (home-subpath "/.deno")
)
...
;; Mix home is read-only: archives and escripts under it are executable code that
;; `mix` loads and that the user may invoke directly outside the sandbox.
;; Installing an archive or escript is a host-side action.
(allow file-read*
    (home-subpath "/.mix")
)
...
;; GOPATH roots are read-only because `go install` drops host-invoked binaries
;; into GOPATH/bin, and goenv keeps managed toolchains and shims under its own
;; root. The module cache under GOPATH/pkg is re-granted below since ordinary
;; builds write there.
(allow file-read*
    (home-subpath "/go")
...
(allow file-read* file-write*
    (home-subpath "/go/pkg")                    ;; GOMODCACHE plus build artifacts written by ordinary `go build`/`go test`.
    (home-subpath "/.local/share/go/pkg")       ;; XDG GOPATH variant of the module cache.
...
;; Installed runtimes and globally installed packages are read-only: agents may
;; execute already-installed Node versions and global CLIs, but must not modify
;; them. A writable global package tree lets a sandboxed agent backdoor a binary
;; the user later runs from an unsandboxed shell.
;; Rules below only match the operations they name, so every writable path
;; nested inside these roots must be re-granted in the read-write block that
;; follows.
(allow file-read*
    ;; Node version managers: managed runtime installs and their bundled global packages.
    (home-subpath "/.nvm")
    (home-subpath "/.fnm")
    ...
    ;; pnpm global bin/package roots (PNPM_HOME).
    (home-subpath "/.local/share/pnpm")
    (home-subpath "/Library/pnpm")

    ;; yarn global bin plus classic `yarn global add` package root.
    (home-subpath "/.yarn")
...
(allow file-read* file-write*
    ...
    (home-subpath "/.local/share/pnpm/store")   ;; Content-addressable store written by ordinary project installs.
    (home-subpath "/Library/pnpm/store")        ;; macOS PNPM_HOME store variant.
    ...
    (home-subpath "/.yarn/berry/cache")     ;; Yarn 4 global download cache, written by ordinary project installs.
    (home-subpath "/.yarn/berry/index")     ;; Yarn Berry install-state index kept alongside the global cache.
    (home-subpath "/.yarn/berry/metadata")  ;; Yarn Berry registry metadata cache.

    ;; corepack
    ;; Package manager payloads here are fetched on demand when a project pins a
    ;; `packageManager` version, so this stays writable for ordinary project work.
...
;; Managed Perl builds and local::lib install trees hold interpreters and
;; module-provided scripts that land on the user's PATH, so they are read-only.
;; perlbrew's switch state and the CPAN client build/caches stay writable.
(allow file-read*
...
(allow file-read* file-write*
    (home-literal "/.perlbrew/init")  ;; Written by `perlbrew switch`/`use` to record the active Perl.
    (home-subpath "/.cpan")
    (home-subpath "/.cpanm")
...
;; The Composer home directories hold `composer global require` packages and
;; their vendor/bin entrypoints, and phpenv holds managed PHP builds plus shims,
;; so these roots are read-only. Composer's download cache is re-granted below.
(allow file-read*
...
(allow file-read* file-write*
    (home-subpath "/.composer/cache")             ;; Composer package cache written by ordinary project installs.
    (home-subpath "/.config/composer/cache")      ;; XDG Composer cache variant.
    (home-subpath "/.local/share/composer/cache") ;; XDG data-dir Composer cache variant.
...
;; Interpreter install roots and globally installed tool binaries are read-only:
;; agents run already-installed interpreters and CLIs but cannot modify one that
;; the user later invokes from an unsandboxed shell. Per-project virtualenvs live
;; in the workdir and are unaffected.
(allow file-read*
    ...
    (home-subpath "/.local/share/uv")   ;; uv-managed interpreters (`uv python install`) and tool venvs (`uv tool install`).
    (home-subpath "/.local/pipx")       ;; pipx-managed virtualenvs and their entrypoint binaries.
...
(allow file-read* file-write*
    ;; uv writes a sibling `.tmp*` lock/staging file directly under tools/ for
    ;; any operation that reads the tool list, even `uv tool list`. This
    ;; prefix only matches that root-level temp file, not the per-tool
    ;; payload directories (e.g. tools/ruff/), so `uv tool install` still
    ;; cannot land a new tool.
    (home-prefix "/.local/share/uv/tools/.tmp")
    (home-literal "/.local/share/uv/tools/.lock")
