...
;; This profile is no longer baseline. It is injected selectively for keychain-dependent
;; agent/app profiles (see bin/lib/policy.sh).
;; Read-only by default. Apps that store credentials in keychain add write grants in their own profiles.
