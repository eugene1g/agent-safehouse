...
# Embedded Profiles Last Modified (UTC): 2026-07-13T15:37:38Z
...
;; Chrome for Testing places its socket dir under the lowercase bundle id
;; com.google.chrome.for.testing.<hash>. Seatbelt path-regex matching is
;; case-insensitive, so the com.google.Chrome.* rule above already matches it,
;; but we list it explicitly so coverage does not silently depend on case-folding.
...
safehouse_project_version_embedded="0.11.0"
