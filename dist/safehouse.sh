...
# Embedded Profiles Last Modified (UTC): 2026-09-05T20:05:59Z
...
PROFILE_KEYS=(
  ...
  "profiles/40-shared/ipc-posix-sem.sb"
  "profiles/40-shared/ipc-sysv-sem.sb"
  ...
  "profiles/55-integrations-optional/gpg.sb"
  "profiles/55-integrations-optional/gpu.sb"
  "profiles/55-integrations-optional/herdr.sb"
  "profiles/55-integrations-optional/keychain.sb"
  "profiles/55-integrations-optional/kubectl.sb"
  "profiles/55-integrations-optional/launch-services.sb"
...
;; ---------------------------------------------------------------------------
;; Base Profile
;; Core definitions, path replacement tokens, and helper macros shared by all modules.
;; Source: 00-base.sb
;; ---------------------------------------------------------------------------
...
;; WORK_DIR is the selected workdir for this policy. Safehouse replaces this
;; line with an absolute string definition, or #f when --workdir= disables it.
;; For a manual policy, replace the marker with (define WORK_DIR "/absolute/path")
;; or (define WORK_DIR #f). Relative helper arguments must begin with "/".
__SAFEHOUSE_REPLACE_ME_WITH_WORK_DIR_DEFINE__

(define (workdir-path rel)
  (string-append (if (string=? WORK_DIR "/") "" WORK_DIR) rel))
(define (workdir-subpath rel) (subpath (workdir-path rel)))
(define (workdir-literal rel) (literal (workdir-path rel)))
(define (workdir-prefix rel) (prefix (workdir-path rel)))
...
(allow file-read*
    ...
    (subpath "/nix/store")                                            ;; Nix package manager store / install root.
    (subpath "/nix/var/nix/profiles")                                 ;; Nix package manager profile symlinks.
...
(allow file-read-metadata
    ...
    (literal "/nix")                                                  ;; Directory traversal for /nix/store path resolution.
...
(allow file-read*
    ...
    (home-literal "/.local/state/nix/profile")                        ;; Current profile symlink for Nix package manager (use-xdg-base-directories layout).
    (home-subpath "/.local/state/nix/profiles")                       ;; Directory of profile symlinks for Nix package manager since 2.14
                                                                      ;; (previously /nix/var/nix/profiles/per-user/<username>).
    (home-literal "/.nix-profile")                                    ;; Current profile symlink for Nix package manager (default layout).
)
...
;; Block inspection of host processes' full argv and environment
;;
;; Empirically (tested on macOS 26), a kern.procargs{,2} sysctl to read another
;; process's argv/env succeeds if either sysctl-read of kern.procargs{,2}.<pid>
;; or process-info-pidinfo is allowed.
;;
;; Re-allow by enabling process-control or lldb.
;;
;; #safehouse-test-id:cross-process-procargs-deny#
(deny sysctl-read (sysctl-name-regex #"procargs"))                    ;; Directly default deny kern.procargs/kern.procargs2 sysctls.
(deny process-info-pidinfo)                                           ;; Also default deny pidinfo (NOT covered by `(default deny)`)
(allow process-info-pidinfo (target same-sandbox))                    ;; In-sandbox, allow pidinfo to reenable the procargs sysctls.
...
;; NOTE: file-read-metadata is named explicitly even though file-read* covers it
;;       so that the specific /private/var/run deny here takes precedence over
;;       the general /private/var/run allow elsewhere in this profile.
;;       Opt-in profiles (like ssh.sb) must allow at the same specificity
;;       to override this deny.
(deny file-read* file-write* file-read-metadata
    (regex #"^/private/tmp/com\.apple\.launchd\.[^/]+/Listeners$")     ;; Real tmp namespace for launchd listener sockets (SSH agent, pasteboard, etc.); Tahoe <= 26.3, Sequoia and earlier.
    (regex #"^/tmp/com\.apple\.launchd\.[^/]+/Listeners$")             ;; Symlinked /tmp view of the same launchd listener sockets.
    (regex #"^/private/var/run/com\.apple\.launchd\.[^/]+/Listeners$") ;; Real var/run namespace for launchd listener sockets on macOS Tahoe 26.4+ (moved from /private/tmp).
    (regex #"^/var/run/com\.apple\.launchd\.[^/]+/Listeners$")         ;; Symlinked /var/run view of the same launchd listener sockets.
)
...
;; Notably excludes communication with UNIX sockets (like docker.sock).
...
;; 
;; Restricting inbound to loopback-only peers is also not a goal here,
;; which does not appear to be possible using Seatbelt. Users who need stricter
;; inbound boundaries should configure macOS Application Firewall or pf.

;; Outbound TCP/UDP connections.
(allow network-outbound (remote ip))
;; DNS resolution via mDNSResponder (required for hostname lookups).
(allow network-outbound (literal "/private/var/run/mDNSResponder"))

;; Inbound TCP/UDP connections.
;; Includes connections to local dev servers on 127.0.0.1 or 0.0.0.0.
...
;; npm global packages directory (used by fnm/XDG prefix setups).
(allow file-read*
    (home-subpath "/.local/lib/node_modules")
)
...
__SAFEHOUSE_EMBEDDED_profiles_40_shared_agent_common_sb__
      ;;
    "profiles/40-shared/ipc-posix-sem.sb")
      cat <<'__SAFEHOUSE_EMBEDDED_profiles_40_shared_ipc_posix_sem_sb__'
;; ---------------------------------------------------------------------------
;; Category: Shared IPC (POSIX semaphores)
;; POSIX named semaphores back cross-process synchronization primitives used by
;; runtimes such as Python's multiprocessing module. Without this allow,
;; sem_open(3) returns EPERM inside the sandbox.
;; Source: 40-shared/ipc-posix-sem.sb
;; ---------------------------------------------------------------------------

(allow ipc-posix-sem)
__SAFEHOUSE_EMBEDDED_profiles_40_shared_ipc_posix_sem_sb__
      ;;
    "profiles/40-shared/ipc-sysv-sem.sb")
      cat <<'__SAFEHOUSE_EMBEDDED_profiles_40_shared_ipc_sysv_sem_sb__'
;; ---------------------------------------------------------------------------
...
(allow ipc-sysv-sem)
__SAFEHOUSE_EMBEDDED_profiles_40_shared_ipc_sysv_sem_sb__
      ;;
    "profiles/50-integrations-core/container-runtime-default-deny.sb")
...
;; classifies as network-outbound — not file-read/write. Deny-by-default network policy
;; in 20-network.sb should block these connects already, so this is defense-in-depth.
...
;; as network-outbound rather than file-read/write. Deny-by-default network policy
;; in 20-network.sb should block these connects already, so this is defense-in-depth.
(deny network-outbound
...
(allow file-read*
    ...
    (home-literal "/Library/Preferences/org.chromium.Chromium.plist")
...
;; $$require=55-integrations-optional/gpu.sb$$
...
(allow file-read* file-write* file-read-xattr file-write-xattr
    ...
    (home-subpath "/Library/Application Support/Google/Chrome for Testing/Crashpad")
    (home-subpath "/Library/Application Support/Chromium/Crashpad")
)

;; Chrome 151+ `--headless=new` creates an isolated
;; "<default-user-data-dir>-headless" container and aborts at startup with
;; "Failed to create headless user data directory container." if it cannot.
(allow file-read* file-write*
    (home-subpath "/Library/Application Support/Google/Chrome-headless")
    (home-subpath "/Library/Application Support/Google/Chrome for Testing-headless")
)

;; Chrome's ProcessSingleton binds, listens, and connects to a UNIX socket.
;; Without these allows, full-browser launch aborts at startup 
;; with error "Failed to bind() ... SingletonSocket".
(allow network-bind network-inbound
    (local unix-socket
        (path-regex #"^(/private)?/var/folders/[^/]+/[^/]+/T/com\.google\.Chrome\.[^/]+/SingletonSocket$"))
    (local unix-socket
        (path-regex #"^(/private)?/var/folders/[^/]+/[^/]+/T/com\.google\.chrome\.for\.testing\.[^/]+/SingletonSocket$"))
    (local unix-socket
        (path-regex #"^(/private)?/var/folders/[^/]+/[^/]+/T/\.org\.chromium\.Chromium\.[^/]+/SingletonSocket$"))
)
(allow network-outbound
    (remote unix-socket
        (path-regex #"^(/private)?/var/folders/[^/]+/[^/]+/T/com\.google\.Chrome\.[^/]+/SingletonSocket$"))
...
;; GPU IOKit user clients (IOSurfaceRootUserClient, AGXDeviceUserClient) are
;; provided by profiles/55-integrations-optional/gpu.sb via $$require above.
(allow iokit-open
...
;; $$require=55-integrations-optional/gpu.sb,55-integrations-optional/macos-gui.sb$$
...
;; Shared with --enable=gpu and chromium-headless via
;; profiles/55-integrations-optional/gpu.sb (injected via $$require above).
...
__SAFEHOUSE_EMBEDDED_profiles_55_integrations_optional_electron_sb__
      ;;
    "profiles/55-integrations-optional/gpg.sb")
      cat <<'__SAFEHOUSE_EMBEDDED_profiles_55_integrations_optional_gpg_sb__'
;; ---------------------------------------------------------------------------
;; Integration: GPG
...
__SAFEHOUSE_EMBEDDED_profiles_55_integrations_optional_gpg_sb__
      ;;
    "profiles/55-integrations-optional/gpu.sb")
      cat <<'__SAFEHOUSE_EMBEDDED_profiles_55_integrations_optional_gpu_sb__'
;; ---------------------------------------------------------------------------
;; Integration: GPU
;; Metal shader compilation and GPU IOKit user clients without the full
;; Electron/macOS-GUI stack. Use for GPU/Metal-only workloads.
;; Source: 55-integrations-optional/gpu.sb
;; #safehouse-test-id:gpu-integration#
;; ---------------------------------------------------------------------------

;; Metal shader compilation service and WebKit platform support helper,
;; used by Chromium/Electron GPU processes and Metal tooling.
(allow mach-lookup
    (global-name "com.apple.MTLCompilerService")
    (global-name "com.apple.SafariPlatformSupport.Helper")
)

;; IOSurface (shared GPU surface buffers) and Apple GPU (AGX) IOKit user
;; clients required for GPU process startup and Metal rendering.
(allow iokit-open
    (iokit-user-client-class "IOSurfaceRootUserClient")
    (iokit-user-client-class "AGXDeviceUserClient")
)

;; GPU helper processes probe setup state at startup.
(allow file-read-metadata
    (literal "/private/var/db/.AppleSetupDone")
)
__SAFEHOUSE_EMBEDDED_profiles_55_integrations_optional_gpu_sb__
      ;;
    "profiles/55-integrations-optional/herdr.sb")
      cat <<'__SAFEHOUSE_EMBEDDED_profiles_55_integrations_optional_herdr_sb__'
;; ---------------------------------------------------------------------------
;; Integration: herdr
;; TUI session status and bell integration for coding agents.
;; Auto-enabled when HERDR_ENV is set in the host environment.
;; Source: 55-integrations-optional/herdr.sb
;; #safehouse-test-id:herdr-integration#
;; ---------------------------------------------------------------------------

(allow file-read*
    (home-subpath "/.config/herdr")                                  ;; herdr config directory containing socket and state files.
)
...
__SAFEHOUSE_EMBEDDED_profiles_55_integrations_optional_kubectl_sb__
      ;;
    "profiles/55-integrations-optional/launch-services.sb")
      cat <<'__SAFEHOUSE_EMBEDDED_profiles_55_integrations_optional_launch_services_sb__'
;; ---------------------------------------------------------------------------
...
;; Source: 55-integrations-optional/launch-services.sb
;; ---------------------------------------------------------------------------

;; WARNING: Processes spawned through Launch Services are created OUTSIDE the
;; current sandbox, with the user's full privileges.
;;
;; This integration is therefore opt-in. Enable it only when a wrapped tool
;; needs `open` (for example Claude Code's /memory -> "Open auto-memory folder")
;; and an unsandboxed launch is acceptable.
...
(allow lsopen)
__SAFEHOUSE_EMBEDDED_profiles_55_integrations_optional_launch_services_sb__
      ;;
    "profiles/55-integrations-optional/lldb.sb")
...
;; #safehouse-test-id:lldb-procargs-allow#
...
;; #safehouse-test-id:process-control-procargs-allow#
(allow process-info-pidinfo)           ;; Host argv/env via kern.procargs{,2} for supervision tools
...
(allow file-read*
    ...
    (home-literal "/.bashrc")                                         ;; Bash interactive startup file read by interactive bash sessions.
    (home-literal "/.bash_profile")                                   ;; Bash profile file read by login bash sessions.
    (home-literal "/.bash_login")                                     ;; Bash login file read if .bash_profile is absent.
    (home-literal "/.bash_logout")                                    ;; Bash logout file sourced when a login shell exits.
...
;; SSH agent socket access via launchd-managed SSH_AUTH_SOCK and Tahoe-style ~/.ssh/agent sockets.
;; NOTE: file-read-metadata is named explicitly to match the specificity of the default deny in
;;       10-system-runtime.sb. A general allow would lose to that specific deny regardless of order.
(allow file-read* file-write* file-read-metadata
    (regex #"^/private/tmp/com\.apple\.launchd\.[^/]+/Listeners$")      ;; launchd-managed SSH_AUTH_SOCK path on macOS (<= Tahoe 26.3, Sequoia and earlier).
    (regex #"^/tmp/com\.apple\.launchd\.[^/]+/Listeners$")              ;; symlinked tmp variant of SSH_AUTH_SOCK path.
    (regex #"^/private/var/run/com\.apple\.launchd\.[^/]+/Listeners$")  ;; launchd-managed SSH_AUTH_SOCK path on macOS Tahoe 26.4+ (moved from /private/tmp).
    (regex #"^/var/run/com\.apple\.launchd\.[^/]+/Listeners$")          ;; symlinked var/run variant of SSH_AUTH_SOCK path.
...
;; Requires: 55-integrations-optional/electron.sb     (crashpad, WebView; transitively pulls gpu + macos-gui)
...
;; Allow a sandboxed VS Code to receive commands
(allow network-bind network-inbound
    (local unix-socket
        (path-regex #"^(/private)?/var/folders/[^/]+/[^/]+/T/vscode-ipc-[0-9A-Fa-f-]+\.sock$")))
;; Disallow sending commands to a potentially unsandboxed VS Code,
;; because IPC protocol gives wide capabilities like opening arbitrary URLs
;; and opening arbitrary local directories with workspace-trust/auto-task behavior
(deny network-outbound
    (remote unix-socket
...
;; Disallow sharing of git credentials from a VS Code (sandboxed or not) to a
;; different party, who cannot be proven to operate within the same trust boundary
(deny network-outbound
    (remote unix-socket
        (path-regex #"^(/private)?/var/folders/[^/]+/[^/]+/T/vscode-git-[0-9a-f]+\.sock$")))
(deny network-bind network-inbound
...
;; Safehouse-managed Electron single-instance socket (VSCODE_IPC_HOOK):
;; <user-data-dir>/<version[0:4]>-main.sock, e.g. user-data/1.12-main.sock.
;; The primary instance binds it; a second launch connects to hand off its full
;; CLI args + environment (open-url deep links, window open) and exit.
;;
;; The Safehouse-managed Claude editor profile keeps its user-data-dir distinct
;; from the user's real VS Code. Allow CLI args to be shared between sandboxed
;; VS Code instances.
(allow network-bind network-inbound
    (local unix-socket
...
;; Unsandboxed Electron single-instance socket (VSCODE_IPC_HOOK):
;; <user-data-dir>/<version[0:4]>-main.sock, e.g. user-data/1.12-main.sock.
;;
;; Disallow sending of arbitrary CLI args across a trust boundary.
(deny network-outbound
    (remote unix-socket
...
safehouse_project_version_embedded="0.11.1"
...
policy_embedded_supported_enable_features="1password, agent-browser, browser-native-messaging, chromium-full, chromium-headless, cleanshot, clipboard, cloud-credentials, cloud-storage, docker, electron, gpg, gpu, herdr, keychain, kubectl, launch-services, lldb, macos-gui, microphone, playwright-chrome, process-control, shell-init, spotlight, ssh, vscode, xcode, all-agents, all-apps, wide-read"
...
policy_dist_emit_embedded_profile_requirement_tokens() {
  case "$1" in
    ...
    "profiles/55-integrations-optional/chromium-full.sb")
      ...
      ;;
    "profiles/55-integrations-optional/chromium-headless.sb")
      printf '%s\n' "55-integrations-optional/gpu.sb"
      ;;
    "profiles/55-integrations-optional/cleanshot.sb")
      :
      ;;
    ...
    "profiles/55-integrations-optional/docker.sb")
      ...
      ;;
    "profiles/55-integrations-optional/electron.sb")
      printf '%s\n' "55-integrations-optional/gpu.sb" "55-integrations-optional/macos-gui.sb"
      ;;
    "profiles/55-integrations-optional/gpg.sb")
      :
      ;;
...
policy_selection_select_matching_command() {
  ...
  case "$command_basename" in
    "aider") profile_key="profiles/60-agents/aider.sb" ;;
    "amp") profile_key="profiles/60-agents/amp.sb" ;;
    "auggie") profile_key="profiles/60-agents/auggie.sb" ;;
    "claude") profile_key="profiles/60-agents/claude-code.sb" ;;
    "claude-code") profile_key="profiles/60-agents/claude-code.sb" ;;
    "cline") profile_key="profiles/60-agents/cline.sb" ;;
    "codex") profile_key="profiles/60-agents/codex.sb" ;;
    "copilot") profile_key="profiles/60-agents/copilot-cli.sb" ;;
    "copilot-cli") profile_key="profiles/60-agents/copilot-cli.sb" ;;
    "cursor-agent") profile_key="profiles/60-agents/cursor-agent.sb" ;;
    "cursor") profile_key="profiles/60-agents/cursor-agent.sb" ;;
    "agent") profile_key="profiles/60-agents/cursor-agent.sb" ;;
    "droid") profile_key="profiles/60-agents/droid.sb" ;;
    "gemini") profile_key="profiles/60-agents/gemini.sb" ;;
    "goose") profile_key="profiles/60-agents/goose.sb" ;;
    "kilo") profile_key="profiles/60-agents/kilo-code.sb" ;;
    "kilocode") profile_key="profiles/60-agents/kilo-code.sb" ;;
    "kilo-code") profile_key="profiles/60-agents/kilo-code.sb" ;;
    "opencode") profile_key="profiles/60-agents/opencode.sb" ;;
    "pi") profile_key="profiles/60-agents/pi.sb" ;;
    *) return 0 ;;
  esac
...
policy_metadata_collect_profile_absolute_path_rules() {
  ...
  case "$2" in
    "profiles/00-base.sb")
      ...
      ;;
    "profiles/10-system-runtime.sb")
      safehouse_array_append "$1" "subpath|/usr" "subpath|/bin" "subpath|/sbin" "subpath|/opt" "subpath|/nix/store" "subpath|/nix/var/nix/profiles" "subpath|/System/Library" "subpath|/System/Volumes/Preboot" "subpath|/Library/Apple" "subpath|/Library/Frameworks" "subpath|/Library/Fonts" "subpath|/Library/Filesystems/NetFSPlugins" "subpath|/Library/Preferences/Logging" "literal|/Library/Preferences/.GlobalPreferences.plist" "literal|/Library/Preferences/com.apple.networkd.plist" "literal|/dev" "literal|/private" "literal|/private/var" "subpath|/private/var/db/timezone" "literal|/private/var/select/sh" "literal|/private/var/select/developer_dir" "literal|/var/select/developer_dir" "literal|/private/var/db/xcode_select_link" "literal|/var/db/xcode_select_link" "literal|/private/etc/hosts" "literal|/private/etc/resolv.conf" "literal|/private/etc/services" "literal|/private/etc/protocols" "literal|/private/etc/shells" "subpath|/private/etc/ssl" "literal|/private/etc/localtime" "subpath|/private/etc" "literal|/etc" "literal|/var" "literal|/dev/zero" "literal|/dev/autofs_nowait" "literal|/dev/dtracehelper" "literal|/dev/urandom" "literal|/dev/random"
      ;;
    "profiles/20-network.sb")
      :
      ;;
    "profiles/30-toolchains/apple-toolchain-core.sb")
      safehouse_array_append "$1" "literal|/Library" "literal|/Library/Developer" "literal|/Library/Developer/CommandLineTools" "literal|/Library/Developer/CommandLineTools/usr" "literal|/Library/Developer/CommandLineTools/usr/bin" "literal|/Library/Developer/CommandLineTools/usr/include" "literal|/Library/Developer/CommandLineTools/usr/lib" "literal|/Library/Developer/CommandLineTools/usr/libexec" "literal|/Library/Developer/CommandLineTools/usr/libexec/git-core" "literal|/Library/Developer/CommandLineTools/usr/share" "literal|/Library/Developer/CommandLineTools/SDKs" "literal|/Library/Developer/CommandLineTools/usr/bin/git" "literal|/Library/Developer/CommandLineTools/usr/bin/make" "literal|/Library/Developer/CommandLineTools/usr/bin/gnumake" "literal|/Library/Developer/CommandLineTools/usr/bin/c++" "literal|/Library/Developer/CommandLineTools/usr/bin/cc" "literal|/Library/Developer/CommandLineTools/usr/bin/clang" "literal|/Library/Developer/CommandLineTools/usr/bin/gcc" "literal|/Library/Developer/CommandLineTools/usr/bin/g++" "literal|/Library/Developer/CommandLineTools/usr/bin/cpp" "literal|/Library/Developer/CommandLineTools/usr/bin/c89" "literal|/Library/Developer/CommandLineTools/usr/bin/c99" "literal|/Library/Developer/CommandLineTools/usr/bin/ar" "literal|/Library/Developer/CommandLineTools/usr/bin/ranlib" "literal|/Library/Developer/CommandLineTools/usr/bin/as" "literal|/Library/Developer/CommandLineTools/usr/bin/ld" "literal|/Library/Developer/CommandLineTools/usr/bin/ld-classic" "literal|/Library/Developer/CommandLineTools/usr/bin/libtool" "literal|/Library/Developer/CommandLineTools/usr/bin/lipo" "literal|/Library/Developer/CommandLineTools/usr/bin/codesign_allocate" "literal|/Library/Developer/CommandLineTools/usr/bin/dsymutil" "literal|/Library/Developer/CommandLineTools/usr/bin/bitcode_strip" "literal|/Library/Developer/CommandLineTools/usr/bin/install_name_tool" "literal|/Library/Developer/CommandLineTools/usr/bin/strip" "literal|/Library/Developer/CommandLineTools/usr/bin/bison" "literal|/Library/Developer/CommandLineTools/usr/bin/byacc" "literal|/Library/Developer/CommandLineTools/usr/bin/flex" "literal|/Library/Developer/CommandLineTools/usr/bin/flex++" "literal|/Library/Developer/CommandLineTools/usr/bin/lex" "literal|/Library/Developer/CommandLineTools/usr/bin/yacc" "literal|/Library/Developer/CommandLineTools/usr/bin/m4" "literal|/Library/Developer/CommandLineTools/usr/bin/gm4" "literal|/Library/Developer/CommandLineTools/usr/bin/bm4" "literal|/Library/Developer/CommandLineTools/usr/bin/gperf" "literal|/Library/Developer/CommandLineTools/usr/bin/ctags" "literal|/Library/Developer/CommandLineTools/usr/bin/mig" "literal|/Library/Developer/CommandLineTools/usr/bin/rpcgen" "literal|/Library/Developer/CommandLineTools/usr/libexec/migcom" "literal|/Library/Developer/CommandLineTools/usr/bin/unifdef" "literal|/Library/Developer/CommandLineTools/usr/bin/unifdefall" "literal|/Library/Developer/CommandLineTools/usr/bin/clang-format" "literal|/Library/Developer/CommandLineTools/usr/bin/clangd" "literal|/Library/Developer/CommandLineTools/usr/bin/c++filt" "literal|/Library/Developer/CommandLineTools/usr/bin/gcov" "literal|/Library/Developer/CommandLineTools/usr/bin/lorder" "literal|/Library/Developer/CommandLineTools/usr/bin/nm" "literal|/Library/Developer/CommandLineTools/usr/bin/objdump" "literal|/Library/Developer/CommandLineTools/usr/bin/otool" "literal|/Library/Developer/CommandLineTools/usr/bin/size" "literal|/Library/Developer/CommandLineTools/usr/bin/strings" "literal|/Library/Developer/CommandLineTools/usr/bin/llvm-cov" "literal|/Library/Developer/CommandLineTools/usr/bin/llvm-cxxfilt" "literal|/Library/Developer/CommandLineTools/usr/bin/llvm-dwarfdump" "literal|/Library/Developer/CommandLineTools/usr/bin/llvm-nm" "literal|/Library/Developer/CommandLineTools/usr/bin/llvm-objdump" "literal|/Library/Developer/CommandLineTools/usr/bin/llvm-otool" "literal|/Library/Developer/CommandLineTools/usr/bin/llvm-profdata" "literal|/Library/Developer/CommandLineTools/usr/bin/llvm-size" "literal|/Library/Developer/CommandLineTools/usr/bin/dyld_info" "literal|/Library/Developer/CommandLineTools/usr/bin/cmpdylib" "literal|/Library/Developer/CommandLineTools/usr/bin/nm-classic" "literal|/Library/Developer/CommandLineTools/usr/bin/otool-classic" "literal|/Library/Developer/CommandLineTools/usr/bin/pagestuff" "literal|/Library/Developer/CommandLineTools/usr/bin/size-classic" "literal|/Library/Developer/CommandLineTools/usr/bin/nmedit" "literal|/Library/Developer/CommandLineTools/usr/bin/segedit" "literal|/Library/Developer/CommandLineTools/usr/bin/vtool" "subpath|/Library/Developer/CommandLineTools/usr/libexec/git-core" "subpath|/Library/Developer/CommandLineTools/usr/include" "subpath|/Library/Developer/CommandLineTools/usr/lib" "subpath|/Library/Developer/CommandLineTools/usr/share" "subpath|/Library/Developer/CommandLineTools/SDKs"
      ;;
    "profiles/30-toolchains/bun.sb")
      :
      ;;
    ...
    "profiles/30-toolchains/go.sb")
      ...
      ;;
    "profiles/30-toolchains/java.sb")
      safehouse_array_append "$1" "subpath|/Library/Java"
      ;;
    "profiles/30-toolchains/node.sb")
      :
      ;;
    ...
    "profiles/30-toolchains/python.sb")
      ...
      ;;
    "profiles/30-toolchains/ruby.sb")
      safehouse_array_append "$1" "literal|/Library" "literal|/Library/Ruby" "subpath|/Library/Ruby/Gems"
      ;;
    "profiles/30-toolchains/runtime-managers.sb")
      :
      ;;
    ...
    "profiles/50-integrations-core/worktrees.sb")
      ...
      ;;
    "profiles/55-integrations-optional/1password.sb")
      safehouse_array_append "$1" "subpath|/Applications/1Password.app"
      ;;
    "profiles/55-integrations-optional/agent-browser.sb")
      :
      ;;
    "profiles/55-integrations-optional/browser-native-messaging.sb")
      ...
      ;;
    "profiles/55-integrations-optional/chromium-full.sb")
      safehouse_array_append "$1" "literal|/Applications" "subpath|/Applications/Google Chrome.app" "literal|/System" "literal|/System/Volumes" "literal|/System/Volumes/Data" "literal|/System/Volumes/Data/Applications" "subpath|/System/Volumes/Data/Applications/Google Chrome.app" "literal|/Library"
      ;;
    "profiles/55-integrations-optional/chromium-headless.sb")
      safehouse_array_append "$1" "literal|/private/etc" "literal|/Library/Preferences/com.apple.HIToolbox.plist"
      ;;
    "profiles/55-integrations-optional/cleanshot.sb")
      :
      ;;
    ...
    "profiles/55-integrations-optional/cloud-storage.sb")
      ...
      ;;
    "profiles/55-integrations-optional/docker.sb")
      safehouse_array_append "$1" "literal|/Applications" "subpath|/Applications/Docker.app" "literal|/System/Volumes/Data" "literal|/System/Volumes/Data/Applications" "subpath|/System/Volumes/Data/Applications/Docker.app" "subpath|/Applications/OrbStack.app/Contents/Resources/completions"
      ;;
    "profiles/55-integrations-optional/electron.sb")
      :
      ;;
    ...
    "profiles/55-integrations-optional/herdr.sb")
      ...
      ;;
    "profiles/55-integrations-optional/keychain.sb")
      safehouse_array_append "$1" "literal|/Library/Preferences/com.apple.security.plist" "literal|/Library/Keychains/System.keychain" "subpath|/private/var/db/mds"
      ;;
    "profiles/55-integrations-optional/kubectl.sb")
      :
      ;;
    "profiles/55-integrations-optional/lldb.sb")
      safehouse_array_append "$1" "literal|/Library" "literal|/Library/Developer" "subpath|/Library/Developer/CommandLineTools" "literal|/Library/Developer/PrivateFrameworks" "subpath|/Library/Developer/PrivateFrameworks" "literal|/Applications" "subpath|/Applications/Xcode.app" "subpath|/Applications/Xcode-beta.app" "literal|/System" "literal|/System/Volumes" "literal|/System/Volumes/Data" "literal|/System/Volumes/Data/Applications" "subpath|/System/Volumes/Data/Applications/Xcode.app" "subpath|/System/Volumes/Data/Applications/Xcode-beta.app"
      ;;
    "profiles/55-integrations-optional/macos-gui.sb")
      safehouse_array_append "$1" "literal|/private/etc" "literal|/private/var/db/eligibilityd/eligibility.plist" "literal|/private/var/db/nsurlstoraged/dafsaData.bin" "literal|/Library/Application Support/Apple/Spotlight/MailUsesCoreSpotlight" "literal|/Library/Preferences/com.apple.HIToolbox.plist"
      ;;
    "profiles/55-integrations-optional/microphone.sb")
      :
      ;;
    ...
    "profiles/55-integrations-optional/process-control.sb")
      ...
      ;;
    "profiles/55-integrations-optional/shell-init.sb")
      safehouse_array_append "$1" "literal|/private/etc/profile" "literal|/private/etc/paths" "subpath|/private/etc/paths.d" "literal|/private/etc/bashrc" "literal|/private/etc/zprofile" "literal|/private/etc/zshrc" "literal|/Applications" "literal|/System/Volumes/Data/Applications" "literal|/private/etc/fish/config.fish" "subpath|/private/etc/fish/conf.d"
      ;;
    "profiles/55-integrations-optional/spotlight.sb")
      :
      ;;
    "profiles/55-integrations-optional/ssh.sb")
      safehouse_array_append "$1" "literal|/private/etc/ssh/ssh_config" "subpath|/private/etc/ssh/ssh_config.d" "literal|/private/etc/ssh/crypto.conf" "subpath|/private/etc/ssh/crypto"
      ;;
    "profiles/55-integrations-optional/vscode.sb")
      :
      ;;
    "profiles/55-integrations-optional/xcode.sb")
      safehouse_array_append "$1" "literal|/Library" "literal|/Library/Developer" "literal|/Library/Developer/CommandLineTools" "subpath|/Library/Developer/CommandLineTools" "literal|/Library/Developer/PrivateFrameworks" "subpath|/Library/Developer/PrivateFrameworks" "literal|/Library/Developer/CoreSimulator" "subpath|/Library/Developer/CoreSimulator" "literal|/Applications" "subpath|/Applications/Xcode.app" "subpath|/Applications/Xcode-beta.app" "literal|/System" "literal|/System/Volumes" "literal|/System/Volumes/Data" "literal|/System/Volumes/Data/Applications" "subpath|/System/Volumes/Data/Applications/Xcode.app" "subpath|/System/Volumes/Data/Applications/Xcode-beta.app" "literal|/Library/Preferences" "literal|/Library/Preferences/com.apple.dt.Xcode.plist"
      ;;
    "profiles/60-agents/aider.sb")
      :
      ;;
    "profiles/60-agents/amp.sb")
      safehouse_array_append "$1" "literal|/home" "literal|/System/Volumes/Data/home"
      ;;
    "profiles/60-agents/auggie.sb")
      :
      ;;
    "profiles/60-agents/claude-code.sb")
      safehouse_array_append "$1" "subpath|/Library/Application Support/ClaudeCode/.claude" "literal|/Library/Application Support/ClaudeCode/managed-settings.json" "literal|/Library/Application Support/ClaudeCode/managed-mcp.json" "literal|/Library/Application Support/ClaudeCode/CLAUDE.md"
      ;;
    "profiles/60-agents/cline.sb")
      :
      ;;
    "profiles/60-agents/codex.sb")
      safehouse_array_append "$1" "subpath|/Applications/ChatGPT.app/Contents/Resources/cua_node" "literal|/Applications/ChatGPT.app/Contents/Resources/codex" "literal|/Library/Preferences/com.openai.codex.plist" "literal|/Library/Managed Preferences/com.openai.codex.plist" "subpath|/etc/codex"
      ;;
    "profiles/60-agents/copilot-cli.sb")
      safehouse_array_append "$1" "subpath|/Applications/Visual Studio Code.app/Contents/Frameworks" "subpath|/Applications/Visual Studio Code - Insiders.app/Contents/Frameworks"
      ;;
    "profiles/60-agents/cursor-agent.sb")
      safehouse_array_append "$1" "literal|/Applications" "literal|/usr/local/bin/cursor" "literal|/opt/homebrew/bin/cursor" "subpath|/Applications/Cursor.app"
      ;;
    "profiles/60-agents/droid.sb")
      :
      ;;
    "profiles/60-agents/gemini.sb")
      safehouse_array_append "$1" "subpath|/Library/Application Support/GeminiCli"
      ;;
    "profiles/60-agents/goose.sb")
      :
      ;;
    "profiles/60-agents/kilo-code.sb")
      safehouse_array_append "$1" "subpath|/Library/Application Support/RooCode"
      ;;
    "profiles/60-agents/opencode.sb")
      :
      ;;
    "profiles/60-agents/pi.sb")
      ...
      ;;
    "profiles/65-apps/claude-app.sb")
      safehouse_array_append "$1" "subpath|/Applications/Claude.app"
      ;;
    "profiles/65-apps/codex-app.sb")
      safehouse_array_append "$1" "subpath|/Applications/Codex.app" "subpath|/Applications/ChatGPT.app"
      ;;
    "profiles/65-apps/cursor-app.sb")
      safehouse_array_append "$1" "literal|/Applications" "subpath|/Applications/Cursor.app"
      ;;
    "profiles/65-apps/vscode-app.sb")
      safehouse_array_append "$1" "literal|/Applications" "subpath|/Applications/Visual Studio Code.app" "subpath|/Applications/Visual Studio Code - Insiders.app"
      ;;
    *)
      policy_metadata_collect_profile_absolute_path_rules_from_source "$@"
      ;;
