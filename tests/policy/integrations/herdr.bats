...
@test "[POLICY-ONLY] herdr profile grants unix-socket access to the whole herdr config dir (default + named-session sockets)" { # https://herdr.dev/docs/socket-api/#socket-paths
  local profile section

  profile="$(safehouse_profile --enable=herdr)"
  section="$(sft_profile_source_section "$profile" "55-integrations-optional/herdr.sb")"

  sft_assert_contains "$section" '(remote unix-socket (home-subpath "/.config/herdr"))'
}
